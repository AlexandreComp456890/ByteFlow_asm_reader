...
export class Add_Immediate implements IInstruction {
    ...
    execute(context: ExecutionContext): void {
        ...
        const result: number | number[] = this.encondingForTheHolyMachine({registers: context.registers, rs: src1, rt: dest, immediate: val2});

        if (Array.isArray(result)){
            console.log();
            for (let i = 0; i < result.length; i++) console.log(ExecutionContext.fixToHex(result[i]));
            console.log();
        }else{
