...
export class Set_On_Less_Than_Immediate_Unsigned {
    constructor() {
        //lw $t0, 
        this.regex = /^\s*sltiu\s+\$(\w+),\s*\$(\w+),\s*([-+]?\w+)\s*$/i;
    }
    match(line) {
    ...
    execute(context) {
        ...
        if (val2 <= 0xFFFF) {
            ...
            console.log(`${ExecutionContext.fixToHex(this.encondingForTheHolyMachine({ registers: context.registers, rs: src1, rt: dest, immediate: val2 }))}\n`);
            return;
        }
